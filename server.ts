...
async function startServer() {
  ...
  app.post("/api/gemini/insert-image", async (req, res) => {
    try {
      const { htmlBlocks, imageName, imageDescription } = req.body;
      ...
      if (!imageName) {
        return res.status(400).json({ error: "Falta el nombre del archivo de imagen." });
      }
      ...
      const prompt = `
Mira este Markdown y dime dónde colocarías esta imagen basado en la descripción proporcionada.
Selecciona una frase o párrafo existente del bloque seleccionado como "anchorText" (ancla de texto), de modo que podamos insertar la imagen antes o después de esa frase.

INFORMACIÓN DE LA IMAGEN/FIGURA:
- Nombre de archivo: "${imageName}"
- Descripción inicial: "${imageDescription || "Sin descripción específica disponible"}"

Escribe un titulo adecuado para la imagen, no inventes datos, limitate a lo que puedas deducir de la descripción proporcionada. Ejemplo: "![Esquema conceptual del proyecto](${imageName}){width=70%}"
...
2. Encuentra una frase o párrafo textual y exacto dentro de ese bloque para usarlo como 'anchorText'. Debe ser un fragmento único e idéntico para que la búsqueda sea exacta.
...
4. Genera la etiqueta Markdown de la imagen exacta en 'imageTag'.
5. Explica brevemente por qué decidiste insertarlo en ese lugar específico.
`;
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              explanation: {
                type: Type.STRING,
                description: "Explicación en español de la inserción y por qué ese lugar es idóneo."
              }
            },
            required: ["selectedBlockId", "anchorText", "relation", "imageTag", "explanation"]
          }
        }
      });
      ...
      const { selectedBlockId, anchorText, relation, imageTag, explanation } = parsedResult;

      // Find the selected block and apply insertion safely
      const targetBlock = htmlBlocks.find((b: any) => b.id === selectedBlockId);
      if (!targetBlock) {
        return res.status(400).json({ error: `La IA seleccionó un bloque inválido (${selectedBlockId}).` });
      }

      const originalCode = targetBlock.code || "";
      let modifiedCode = "";
